...
class Zstd__Stream__KnownFinalSize
{

public:


    Zstd__Stream__KnownFinalSize()
    {
...
public:
    ...
    const result_flag run_StartStream(const int num_thread, const int level_compressed, const size_t FinalySize, const size_t CHUNK_SIZE, const char* PointerToData_WhichShouldBe_Compressed, const size_t DataSize_WhichShouldBe_Compressed, std::string& string_CompressedResult, const Stream_flag User_Stream_flag)
    {
...
private:
    ...
    const result_flag Run_Uncompress_(const size_t CHUNK_SIZE, const size_t size_BeforeResize, const char* PointerToData_WhichShouldBe_Uncompressed, const size_t DataSize_WhichShouldBe_Uncompressed, std::string& string_UnUncompressedResult)
    {
        ...
        //------------------------------------------------------
        try { string_UnUncompressedResult.resize(size_BeforeResize + DataSize_WhichShouldBe_Uncompressed); } catch (const std::bad_alloc& e) { ErrorName = e.what(); return  result_flag::bad_alloc; }  //Выделяем память под выходной буффер, в который будет записыватся частичный результат сжатия.
        //------------------------------------------------------
